        ...
        if re.match(r'#\w', chan):
            if data.find('::') != -1:
                ...
                # Calculate
                    ...
                    curl = os.popen('curl -H \"Authentication: token TOKEN\" -H \"Accept: application/vnd.github.mercy-preview+json\" https://api.github.com/search/repositories\?q\=%s\+language:%s' % (insert, lang)).read()
                    try:
